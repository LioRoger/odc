...
public class DefaultJobEventListener implements JobEventHandler {
    ...
    @Override
    public void afterStartJob(ExecutorIdentifier executorIdentifier, JobContext jobContext) throws JobException {
        JobConfiguration jobConfiguration = JobConfigurationHolder.getJobConfiguration();
        TaskFrameworkService taskFrameworkService = jobConfiguration.getTaskFrameworkService();
        int rows = taskFrameworkService.startSuccess(jobContext.getJobIdentity().getId(), executorIdentifier.toString(), jobContext);
        if (rows <= 0) {
            throw new JobException("Update job status to RUNNING failed, jobId={0}.", jobContext.getJobIdentity().getId());
        }
    }

    @Override
    public void afterFinished(ExecutorIdentifier executorIdentifier, JobContext jobContext) throws JobException {
        JobConfiguration jobConfiguration = JobConfigurationHolder.getJobConfiguration();
        TaskFrameworkService taskFrameworkService = jobConfiguration.getTaskFrameworkService();
        ...
        if (rows > 0) {
            ...
            throw new JobException("Update executor to destroyed failed, JodId={0}", jobContext.getJobIdentity().getId());
        }
    }

    @Override
    public void finishFailed(ExecutorIdentifier executorIdentifier, JobContext jobContext) {
