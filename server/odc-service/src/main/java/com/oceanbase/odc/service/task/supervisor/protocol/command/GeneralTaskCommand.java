...
package com.oceanbase.odc.service.task.supervisor.protocol.command;
...
public class GeneralTaskCommand extends TaskCommand {
    ...
    public static <T extends GeneralTaskCommand> T fromJsonNode(Supplier<T> commandSupplier, JsonNode jsonNode, CommandType commandType) {
        T command = commandSupplier.get();
        JsonNode executorEndpointNode = jsonNode.get(EXECUTOR_ENC_POINT_STR);
        ExecutorEndpoint endpoint = JsonUtils.fromJson(null == executorEndpointNode ? null : executorEndpointNode.asText(), ExecutorEndpoint.class);
